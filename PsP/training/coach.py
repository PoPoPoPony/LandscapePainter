...
class Coach:
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			for batch_idx, batch in enumerate(self.train_dataloader):
				...
				img, anno = batch
				img, anno = img.to(self.device), anno.to(self.device)
				anno = convertAnnoTensor(anno, 1399)
				
				img = img.float()
				anno = anno.float()
				y_hat, latent = self.net.forward(anno, return_latents=True)

				loss, loss_dict, id_logs = self.calc_loss(anno, img, y_hat, latent)
				loss.backward()
				self.optimizer.step()

				# Logging related
				if self.global_step % self.opts.image_interval == 0 or (self.global_step < 1000 and self.global_step % 25 == 0):
					self.parse_and_log_images(id_logs, anno, img, y_hat, title='images/train/faces')
				...
				if self.opts.use_wandb and batch_idx == 0:
					self.wb_logger.log_images_to_wandb(anno, img, y_hat, id_logs, prefix="train", step=self.global_step, opts=self.opts)
	...
	def configure_datasets(self):
		...
		dataset_args = data_configs.DATASETS[self.opts.dataset_type]
		transforms_dict = dataset_args['transforms'](self.opts).get_transforms()
		train_dataset = ADE20KDS("ADE20K_2021_17_01")
		if self.opts.use_wandb:
			self.wb_logger.log_dataset_wandb(train_dataset, dataset_name="Train")
		print(f"Number of training samples: {len(train_dataset)}")
	...
	def calc_loss(self, x, y, y_hat, latent):
		...
		if self.opts.l2_lambda > 0: # L2 loss
			loss_l2 = F.mse_loss(y_hat, y)
			loss_dict['loss_l2'] = float(loss_l2)
			loss += loss_l2 * self.opts.l2_lambda
		if self.opts.lpips_lambda > 0: # lpip loss
		...
		if self.opts.w_norm_lambda > 0: # regularization loss
